...
var context$2 = require('./context-621bbefc.cjs.prod.js');
...
const SelectMenuTrigger = /*#__PURE__*/React.forwardRef(function SelectMenuTrigger({
  children,
  className = '',
  classNames = {},
  iconEnd,
  ...
  const context$3 = context$1.useDropdownContext();
  const {
    triggerProps
  } = useSelectMenuTriggerProps(context$3);
  const contentLabel = React.useId();
  const {
    selectedOption
  } = context$2.useSelectMenuContext();
  const defaultIconEnd = context$3.isMenuOpen ? dropdownUpIcon.DropdownUpIcon : dropdownDownIcon.DropdownDownIcon;
  const ChildrenOrPlaceholder = React.useCallback(() => {
    if (selectedOption || children) return children;
    return (
      /*#__PURE__*/
  ...
  }, [children, placeholder, selectedOption]);
  const styleProps = index.getStyleProps({
    className,
    classNames,
    style,
    styles: styles$1,
    tw: styles.getSelectMenuTriggerStyles({
      size
    })
  });
...
function useSelectMenuTriggerProps({
  activeDescendantId,
  descendantNodes,
  goToFirstMenuItem,
  goToLastMenuItem,
...
exports.SelectMenuTrigger = SelectMenuTrigger;
exports.useSelectMenuTriggerProps = useSelectMenuTriggerProps;
