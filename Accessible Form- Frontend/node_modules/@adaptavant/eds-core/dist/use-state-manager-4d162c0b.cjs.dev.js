...
require('lodash');
require('./parse-image-processing-options-c3eac627.cjs.dev.js');

const parseImageBlob = blobUrl => {
  try {
    const {
      url
    } = processLocalFile.createUrl(blobUrl);
...
const parseImageUrl = imageUrl => {
  try {
    const {
      url
    } = processLocalFile.createUrl(imageUrl);
...
const useChangesSaver = ({
  ...
  const saveChanges = use_deep_compare_memoize.useDeepCompareCallback(async () => {
    ...
    try {
      ...
      processLocalFile.handleTryCatchError({
        error,
        onError,
...
const useLocalImageProcessor = ({
  ...
  const processLocalImage = use_deep_compare_memoize.useDeepCompareCallback(async file => {
    setIsProcessingLocalFile(true);
    const {
      blob,
      errorMessage
    } = await processLocalFile.processLocalFile(file, translations, imageProcessingOptions, onError);
    if (!blob) {
...
const useRemoteImageLoader = ({
  ...
  const loadRemoteImage = use_deep_compare_memoize.useDeepCompareCallback(async url => {
...
const useStateManager = ({
  ...
  const [isLoadingRemoteImage, setIsLoadingRemoteImage] = React.useState(false);
  const [isProcessingLocalFile, setIsProcessingLocalFile] = React.useState(false);
  ...
  const clearErrorMessage = React.useCallback(() => setErrorMessage(undefined), []);
  const {
    processLocalImage
  ...
  const updateCropDimensions = React.useCallback(newCropDimensions => {
  ...
  const resetOnClose = React.useCallback(() => {
