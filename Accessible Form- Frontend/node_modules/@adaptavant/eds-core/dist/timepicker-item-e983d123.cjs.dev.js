...
/**
 * Item to be rendered inside of a TimePickerListBox.
 */
const TimePickerItem = forwardRef.forwardRefWithGenericComponent(function TimePickerItem({
  children,
  className = '',
  classNames = {},
  id: idProp,
  ...
  // check if selectedOption is from the timeoptionslist to remove
  // addditional space for tick
  const isSelectedFromOptions = React.useMemo(() => {
    return Boolean(selectedOption && timeOptions.some(option => option.time === selectedOption.time));
  ...
  const handleOnClick = React.useCallback(event => {
    ...
    closeMenu();
  }, [closeMenu, onClick, onSelectionChange, option]);
  const railStart = isSelectedFromOptions ? /*#__PURE__*/jsxRuntime.jsx(align_child_to_text.AlignChildToText, {
  ...
  const styleProps = index.getStyleProps({
    ...
    style,
    styles: styles$1,
    tw: styles.getComboboxItemStyles(isSelectedFromOptions)
  });
...
exports.TimePickerItem = TimePickerItem;
